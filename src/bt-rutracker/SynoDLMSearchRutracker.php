...
if (!class_exists('SynoDLMSearchRutrackerItem')) {
    include(__DIR__ . DIRECTORY_SEPARATOR . 'SynoDLMSearchRutrackerItem.php');
}
...
class SynoDLMSearchRutracker
{
     ...
     * @access private
     */
    private $logsPath = '/tmp/bt-rutracker.log';
     ...
     * @access protected
     */
    protected $pagePrefix = 'http://rutracker.org/forum';
     ...
     * @access protected
     */
    protected $queryUrl = 'http://rutracker.org/forum/tracker.php?nm=%s';
     ...
     * @access protected
     */
    protected $loginUrl = 'http://login.rutracker.org/forum/login.php';
     ...
     * @access protected
     */
    protected $cookiePath = '/tmp/bt-rutracker.cookie';
    ...
    public function prepare($curl, $query, $username = null, $password = null)
    {
        ...
        if (strpos($query, '[opt:') !== false) {
            ...
            if (isset($matches['host']) && strpos($matches['host'], 'rutracker') === false) {
    ...
    public function VerifyAccount($username, $password)
    {
        ...
        return ($this->getQuery($content)->find('#login-form .warnColor1')->length ? false : true);
    }
    ...
    private function loadPages($response)
    {
        ...
        preg_match(
            '/(?P<result>\d+)\s+\(/',
            $this->getQuery($response)->find('#main_content table .vBottom .med:first')->text(),
            $matches
        );
        ...
        if ($numLoadPages > 1) {
            ...
            $i = 2;
            foreach ($this->getQuery($response)->find('#main_content .vBottom .pg-jump-menu ~ a:not(:last)') as $page) {
                if ($i > $numLoadPages) {
                    break;
                }
    ...
    {
        $findNum = 0;
        foreach ($this->getQuery($response)->find('.forumline tr.hl-tr') as $item) {
        ...
        if (!$this->singleMode && $this->loadPages($response)) {
            ...
            foreach ($this->linksPage as $pageLink => $item) {
                ...
                foreach ($content->find('#topic_main .row1:eq(0) .post_wrap .post_body') as $node) {
                    $torrent = new SynoDLMSearchRutrackerItem($pageLink, pq($item), $content, pq($node));
