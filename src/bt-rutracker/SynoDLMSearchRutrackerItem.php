 ...
 */
class SynoDLMSearchRutrackerItem extends SynoDLMSearchRutracker
{
    ...
    {
        if (empty($this->title)) {
            $title = $this->content->find('#main_content table .vBottom .maintitle a:first')->text();
    ...
    public function getDownload()
    {
        if (empty($this->download)) {
            $download = $this->node->find('#tor-reged .attach a.dl-link')->attr('href');
    ...
    public function getCategory()
    {
        if (empty($this->category)) {
            $category = $this->content->find('#main_content table .vBottom table .nav a:last')->text();
    ...
    {
        if (empty($this->size)) {
            $this->size = (float)$this->item->find('td.tor-size u')->text();
    ...
    {
        if (empty($this->seeds)) {
            $seeds = $this->item->find('td b.seedmed')->text();
    ...
    public function getLeeches()
    {
        if (empty($this->leeches)) {
            $leeches = $this->item->find('td.leechmed b')->text();
