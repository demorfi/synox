...
if (!class_exists('SynoDLMSearchRutrackerItem')) {
    include(__DIR__ . DIRECTORY_SEPARATOR . 'SynoDLMSearchNNMClubItem.php');
}

/**
 * Synology Download Station Search File.
 * For search torrent files to nnm-club.me.
 *
...
class SynoDLMSearchNNMClub
{
     ...
     * @access private
     */
    private $logsPath = '/tmp/bt-nnmclub.log';
     ...
     * @access protected
     */
    protected $pagePrefix = 'http://nnm-club.me/forum';
     ...
     * @access protected
     */
    protected $queryUrl = 'http://nnm-club.me/forum/tracker.php?nm=%s';
     ...
     * @access protected
     */
    protected $loginUrl = 'http://nnm-club.me/forum/login.php';
     ...
     * @access protected
     */
    protected $cookiePath = '/tmp/nnmclub.cookie';
    ...
    public function prepare($curl, $query, $username = null, $password = null)
    {
        ...
        if (strpos($query, '[opt:') !== false) {
            ...
            if (isset($matches['host']) && strpos($matches['host'], 'nnmclub') === false) {
    ...
    public function VerifyAccount($username, $password)
    {
        ...
        return ($this->getQuery($content)->find('.row1 .gen a[href^=login]')->length ? false : true);
    }
    ...
    private function loadPages($response)
    {
        ...
        preg_match(
            '/(?P<result>\d+)\s+\(max/is',
            $this->getQuery($response)->find('#search_form table .nav')->text(),
            $matches
        );
        ...
        if ($numLoadPages > 1) {
            ...
            $i = 2;
            foreach ($this->getQuery($response)->find('.forumline ~ table .nav b ~ a:not(:last)') as $page) {
                if ($i > $numLoadPages) {
                    break;
                }

                curl_setopt($curl, CURLOPT_URL, $this->pagePrefix . '/' . html_entity_decode(pq($page)->attr('href')));
    ...
    {
        $findNum = 0;
        foreach ($this->getQuery($response)->find('.forumline.tablesorter tbody tr') as $item) {
            $pageLink                   = $this->pagePrefix . '/' . pq($item)->find('.genmed a.genmed')->attr('href');
    ...
    public function parse($plugin, $response)
    {
        ...
        if (!$this->singleMode && $this->loadPages($response)) {
            ...
            foreach ($this->linksPage as $pageLink => $item) {
                ...
                foreach ($content->find('.forumline:last .row1:first > .row1:last') as $node) {
                    $torrent = new SynoDLMSearchNNMClubItem($pageLink, pq($item), $content, pq($node));
