 ...
 */
class SynoDLMSearchNNMClubItem extends SynoDLMSearchNNMClub
{
    ...
    {
        if (empty($this->title)) {
            $title = $this->item->find('.genmed a.genmed')->text();
    ...
    public function getDownload()
    {
        if (empty($this->download)) {
            $download = $this->node->find('.btTbl .gensmall .genmed a:first')->attr('href');
    ...
    public function getCategory()
    {
        if (empty($this->category)) {
            $category = $this->item->find('td:eq(1) a.gen')->text();
    ...
    public function getDateTime()
    {
        if (empty($this->datetime)) {
            $date = $this->item->find('td:last u')->text();
    ...
    {
        if (empty($this->size)) {
            $this->size = (float)$this->item->find('td.gensmall:not(:last) u')->text();
    ...
    public function getLeeches()
    {
        if (empty($this->leeches)) {
            ...
            $this->debug('item find leechs ' . $this->leeches);
        }
