...
if (!class_exists('SynoDLMSearchTpbItem')) {
    include(__DIR__ . DIRECTORY_SEPARATOR . 'SynoDLMSearchTpbItem.php');
}
...
class SynoDLMSearchTpb
{
     ...
     * @access private
     */
    private $logsPath = '/tmp/bt-tpb.log';
     ...
     * @access protected
     */
    protected $pagePrefix = 'https://thepiratebay.se';
     ...
     * @access protected
     */
    protected $queryUrl = 'https://thepiratebay.se/search/%s/%d/7/';
     ...
     * @access protected
     */
    protected $loginUrl = 'https://thepiratebay.se/login';
     ...
     * @access protected
     */
    protected $cookiePath = '/tmp/bt-tpb.cookie';
    ...
    public function prepare($curl, $query, $username = null, $password = null)
    {
        ...
        if (strpos($query, '[opt:') !== false) {
            ...
            if (isset($matches['host']) && strpos($matches['host'], 'tpb') === false) {
        ...
        curl_setopt($curl, CURLOPT_COOKIE, 'language=ru_RU;');
    ...
    public function VerifyAccount($username, $password)
    {
        ...
        return ($this->getQuery($content)->find('.red ~ .loginForm')->length ? false : true);
    }
    ...
    private function loadPages($response)
    {
        // total search result
        preg_match(
            '/\d+\s+\(из\s+(?P<result>\d+)\s+/is',
            $this->getQuery($response)->find('#header ~ h2')->text(),
            $matches
        );
    ...
    {
        $findNum = 0;
        foreach ($this->getQuery($response)->find('#searchResult tr:not(.header)') as $item) {
            $pageLink                   = $this->pagePrefix . pq($item)->find('a.detLink')->attr('href');
    ...
    public function parse($plugin, $response)
    {
        ...
        if (!$this->singleMode && $this->loadPages($response)) {
            ...
            foreach ($this->linksPage as $pageLink => $item) {
                ...
                foreach ($content->find('#main-content #details') as $node) {
                    $torrent = new SynoDLMSearchTpbItem($pageLink, pq($item), $content, pq($node));
